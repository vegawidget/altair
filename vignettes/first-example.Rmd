...
Here's where [reticulate](https://rstudio.github.io/reticulate/) does its job. It exposes Python objects as an R [Reference Class](http://adv-r.had.co.nz/OO-essentials.html#rc) object. Here, this means that wherever you see a `.` in the Altair Python documentation, use a `$` in your R code:
...
At this point, you have an Altair chart. You can poke around a bit to see what it "is":

```{r}
class(plot)
```

You can examine the chart by printing it (supressing the output here because it's a bit verbose):
...
Altair recognizes [four types of data](https://altair-viz.github.io/user_guide/encoding.html#data-types), `"quantitative"`, `"nominal"`, `"ordinal"`, and `"temporal"`. As a side note, the variable `cyl` could be called `"ordinal"`, but I think the default color scale for `"nominal"` works better in this case. It remains an exercise for the author to figure out how to specify color scales.
