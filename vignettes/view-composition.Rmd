...
Vegalite and Altair have several methods to help combine different views of these data.  
...
vegalite(scatter_plot + line_plot, height = 400)
```
...
hist_plot <- alt$Chart(
  r_to_py(beaver1)
    ...
    alt$X("temp:Q", bin=TRUE),
    alt$Y(aggregate='count', type='quantitative', field = alt$Undefined)
...
vegalite((combined_plot | hist_plot) & (combined_plot2 | hist_plot2), height = 600)
```
...
vegalite(plot$facet(row = "cyl:N", column = "gear:N"), height = 600)
```
...
Here is an example of creating plots iwth different x and y axes for the mtcars dataset:
...
vegalite(plot2, height = 450)
```

## More Information

For more information on view composition in vegalite and Altair, check out the [vegalite](https://vega.github.io/vega-lite/docs/composition.html) or [Altair](https://altair-viz.github.io/user_guide/compound_charts.html#) documentation about the subject!
