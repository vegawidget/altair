...
```
So far, so good: the two charts have different memory addresses. This suggests that Altair methods use copy-on-modify semantics, as we are used-to in R.
...
Yikes! It looks like assignment of Altair objects (and perhaps, be extension, Pyhton objects) return a reference, rather than a copy, which we are not used-to in R.

To get the the behavior we are used-to, use a copy method.
