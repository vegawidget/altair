...
1. Give R a hint for which Python environment to use. Put `reticulate::use_condaenv("r-reticulate")` into the `.First()` function in your `.Rprofile` file. If you do this, do not set `RETICULATE_PYTHON` in your `.Renviron` file. As usual, restart R for changes to take effect.
1. `devtools::install_github("ijlyttle/altair")`
...
If you have any SSL-related difficulties in the process, please see the [**Proxies and certificates**](#proxies) section of this article. This may be the case if you are working in a corporate or institutional environment.
...
Thanks to the folks at RStudio, once you have a "standard" Python environment working, each subsequent installation is a single command-line call in addition to installing the R package. 

If you have a working Python (Conda recommended) environment called `"r-reticulate"`, you can skip to the [**Altair installation**](#altair) section of this article.
