...
- You can examine a chart-specification using the `vw_examine()` function, which wraps `listviewer::jsonedit()`.
...
You might have noticed that we did not set the size of the chart anywhere, to see where this is specified within the chart, we can use the `vw_examine()` function: 
