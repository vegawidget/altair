...
- In RMarkdown, the `vegawidget()` `width` and `height` can be set using the knitr options `vega.width` and `vega.height`, subject to the single-view constraint described above.
...
1. You [cannot specify "autosize"](https://vega.github.io/vega-lite/docs/size.html#limitations), i.e. you cannot specify the overall dimensions, for any faceted, repeated, or concantenated chart.
...
In RMarkdown, you can set the `width` and `height` arguments of `vegawidget()` using the knitr options `vega.width` and `vega.height`. Of course, you are still subject to the "autosize" caveats outlined above.
