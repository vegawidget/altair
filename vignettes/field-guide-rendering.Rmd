...
### Using the JSON data backend for Altair

Rather than embed the data itself into the chart spec as JSON (this will become very slow as the data gets to be more than 5000 rows), Altair has an option (supported by Vegalite's url loader) that saves your data as a JSON file that is referenced in the chart spec.
To use it in Python, you do would set `alt.data_transformers.enable('json')` so in R this becomes:
...
This is calling the print method on chart, which runs `vegawidget(chart)`, but instead you need to call it like:

```
vegawidget(chart, base_url='.')
```

and you'll see the chart now!
